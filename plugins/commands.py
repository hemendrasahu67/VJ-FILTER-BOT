...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('❤️MOVIES REQUEST GROUP ❤️', url="https://t.me/DIAMOND_MOVIIES_REQUEST"),
            InlineKeyboardButton('🔥Mᴏᴠɪᴇ Gʀᴏᴜᴘ🔥', url='https://t.me/DIAMOND_MOVIIES')
        ],[
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAKBlmcEH9zzLz9NoeAcUYUuwrDyycFgAAIvAAO5Al4ueFHVK-yWR2w2BA-yWR2w2BA") 
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('💗 OWNER 💗', url="https://t.me/EX_DIAMOND")
                    ],[
        ...
        try:
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton("💗 OWNER 💗", url="t.me/EX_DIAMOND")
                ],[
                    InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @KingVJ01
                ]]
            else:
            ...
            title = '@DIAMOND_MOVIESS_BOT  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_
    title = '@DIAMOND_MOVIESS_BOT  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files["file_name"].split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@DIAMOND_MOVIESS_BOT  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files['file_name'].split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
...
@Client.on_message(filters.command("remove_premiummm"))
...
@Client.on_message(filters.command("plannn"))
...
@Client.on_message(filters.command("myplannn"))
